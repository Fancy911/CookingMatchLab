...
describe('CP0-R0 fixed research sessions RS01-RS05', () => {
  it('RS01_TUTORIAL_REPEAT 同菜重复、计数1→2且首次奖励只发一次', () => {
    ...
    const run = runR0ScenarioCase(registry, scenario, scenario.cases[0]);
  ...
  it('RS05_TIMER_END 5份自动开火与3份半成品两个子用例均稳定', () => {
    ...
    const formal = runR0ScenarioCase(registry, scenario, scenario.cases[0]);
    const partial = runR0ScenarioCase(registry, scenario, scenario.cases[1]);
