...
const resultPath = join(outputDirectory, 'CP0R-R0-Cocos-Build-Result.json');
const logPath = join(outputDirectory, 'CP0R-R0-Cocos-Build-3.8.8.log');
const creatorBinary = process.env.COCOS_CREATOR_BIN
  ?? '/Applications/CocosCreator.app/Contents/MacOS/CocosCreator';
const buildArgument =
  'platform=web-mobile;debug=false;buildPath=project://build;outputName=cp0r-r0-web-mobile';
...
const keyLines = sanitized.split('\n').filter((line) =>
  /Arguments:|project: <PROJECT_ROOT>|Version information looks good|Register CP0ABattleShell|engineVersion="3\.8\.8"|Build Assets success|build Task \(cp0r-r0-web-mobile\) Finished/.test(line));
...
const verification = {
  ...
  buildFinished: /build Task \(cp0r-r0-web-mobile\) Finished/.test(raw),
...
writeFileSync(logPath, [
  '# CP0-R0 Cocos Creator build evidence',
...
console.log(`CP0-R0 Cocos build evidence: ${record.status}`);
