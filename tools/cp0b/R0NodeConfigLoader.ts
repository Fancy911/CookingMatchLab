...
import {
  R0ConfigRegistry,
} from '../../assets/game/scripts/application/cp0c/R0ConfigRegistry';

const readJson = (path: string): unknown =>
  JSON.parse(readFileSync(path, 'utf8')) as unknown;

export const defaultR0ConfigDirectory = (): string =>
  join(process.cwd(), 'assets', 'resources', 'game', 'config', 'cp0-b');

export const loadR0ConfigRegistry = (
  directory = defaultR0ConfigDirectory(),
): R0ConfigRegistry => {
  const scenarioDirectory = join(directory, 'scenarios');
  return R0ConfigRegistry.fromRaw({
    ...
    scenarios: readdirSync(scenarioDirectory)
      .filter((file) => /^RS0[1-5].*\.json$/.test(file))
      .sort()
