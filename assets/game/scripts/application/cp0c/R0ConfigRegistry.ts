import { R0RecipeResolver, type R0ConfigBundle } from '../../domain/cp0b/core';
...
export interface R0RawConfigData {
  gameplay: unknown;
  ingredients: unknown;
  recipes: unknown;
  orders: unknown;
...
export class R0ConfigRegistry implements R0ConfigBundle {
  ...
  public readonly recipeById: Map<RecipeId, R0RecipeConfig>;
  public readonly orderById: Map<string, R0OrderConfig>;
  public readonly scenarioById: Map<string, R0ScenarioConfig>;
  public readonly configHash: string;

  public constructor(
    public readonly gameplay: R0GameplayConfig,
    public readonly ingredients: IngredientConfig[],
    public readonly recipes: R0RecipeConfig[],
    public readonly orders: R0OrderConfig[],
    public readonly tutorials: TutorialConfig[],
    public readonly scenarios: R0ScenarioConfig[],
  ) {
  ...
  public static fromRaw(raw: R0RawConfigData): R0ConfigRegistry {
    ...
    return new R0ConfigRegistry(
      gameplayRoot as unknown as R0GameplayConfig,
      array(ingredientsRoot.ingredients, 'ingredients.ingredients') as IngredientConfig[],
      array(recipesRoot.recipes, 'recipes.recipes') as R0RecipeConfig[],
      array(ordersRoot.orders, 'orders.orders') as R0OrderConfig[],
      array(tutorialsRoot.tutorials, 'tutorials.tutorials') as TutorialConfig[],
      raw.scenarios as R0ScenarioConfig[],
    );
  }
  ...
  private validateScenarios(): void {
    ...
    this.scenarios.forEach((scenario) => {
      ...
      cases.forEach((rawCase, caseIndex) => {
        ...
        const actions = array(testCase.actions, `${caseLabel}.actions`) as R0ScenarioAction[];
        if (actions.length === 0) {
  ...
  private validateScenarioAction(action: R0ScenarioAction, label: string): void {
  ...
  private validateRecipeDeterminism(): void {
    const resolver = new R0RecipeResolver(this.recipes);
